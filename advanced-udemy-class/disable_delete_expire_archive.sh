...
# This script disables/deltes/archives the local user.
# To test the script you can run `create_user_4test.sh` in the first place.

readonly ARCHIVE_DIR='/archive'

# Function to display the usage info.
usage() {
	echo "Usuage: ${0} [-dra] USER [USERN]..." >&2
...
do	
	...
	then	
		...
		then
			...
			mkdir -p ${ARCHIVE_DIR} # -p creates even if the parent directory does not exists.
			if [[ "${?}" -ne 0 ]]
			then
	...
	then
		...
		# We don't want to tell the user that an account was diasbled when it hasn't been.
		if [[ "${?}" -ne 0 ]]
		then
